...
def formataDot(nomearq,formatoSaida):    
    dotCmd =  "dot -T%s %s -O" % (formatoSaida,nomearq)
    return dotCmd

def executaDot(dirarqs):
    cwd = os.getcwd()
    os.chdir(dirarqs)
    
    arqs_dot =  glob.glob("*.dot")
    arqs_dot.sort()

    logNow()
    for arq in arqs_dot:
        try:
            cmdDot = formataDot(arq, "svg")
            print("> %s" % cmdDot)
            subprocess.call(cmdDot, stdout=lf, stderr=lf, shell=True)
        except OSError as _:
            print("ERRO: processando arquivo dot: %s" % arq)
        
    os.chdir(cwd)

def formataComando(dir_tst, dir_saida, prog,arq, extraArgs="",ecpm=True,comVias=True,comBd=True,criarBd=True, 
                   nomebd="",extraExecPars=""): 
    ...
    lnc = "%s %s %s %s -f %s %s %s %s %s" % (prog,extraArgs,bed,bsd,barq,ecpmStr,viasStr,extraExecPars,bdStr)

    return (lnc,path,nomearq)
    
def execTeste(dir_tst, dir_saida, prog, arqgeo, extraPars="",ecpm=False,comVias=False, comBd=False, extraExecPars=""):
    ...
    print("    > extraExecPars: " + extraExecPars)
...
#    totGeos = len(arqs_geo)
    ...
    for arq in arqs_geo:
        ...
        (lnc,path,nomearq) = formataComando(dir_tst, dir_saida, prog,arq,extraPars,ecpm,False,False,False,"",extraExecPars)
        ...
        if os.path.isdir(dirqry):
            ...
            (lnc,path,nomearq) = formataComando(dir_tst, dir_saida, prog,arq, extraPars,ecpm,comVias,False,False,"",extraExecPars)
...
def testaSubmissao(aluno_d,testes_dir, arqgeo, extraPars="", ecpm=False,comVias=False, comBd=False, execDot=False, extraExecPars=""):
    adir_o = os.path.join(aluno_d,"o")
    adir_ot6 = os.path.join(aluno_d,"src",nomeExec)
    execTeste(testes_dir, adir_o,    adir_ot6, arqgeo, extraPars,ecpm,comVias, comBd,extraExecPars)
...
def processaTodosZip(zip_d,teste_d,home_d,arqgeo,alunosd="alunos",extraPars="",ecpm=False,comVias=False, comBd=False,subdirs=[], 
    ...
    if testar:
        ...
        sys.stdout.flush()
        sys.stderr.flush()
        testaSubmissao(os.path.join(alunos_d,saluno),teste_d,arqgeo,extraPars,ecpm,comVias, comBd, comDot,extraExecPars)
...
def executa():
    ...
    arqgeo = "a.geo"
    try:
        opts, _ = getopt.getopt(sys.argv[1:], "zZcthp:IE:X:g:mvDK:", [])
        for opt, arg in opts:
            if opt == '-h':
                uso()
                sys.exit()
...
# para executar dot: -D
#
# para passar parametros extras para o executavel: -K "par1 par2 par3"
#
# EXEMPLO:
#
#  python3 et.py -E ted -z -c -p zemane -K "-ns 32 -fd 75" -g t01.geo
